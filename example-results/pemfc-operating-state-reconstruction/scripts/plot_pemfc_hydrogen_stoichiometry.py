#!/usr/bin/env python3
"""Standalone plot — run with: python plot_pemfc_hydrogen_stoichiometry.py [-o output.svg]
...
# ---------------------------------------------------------------------------
VIPR_STYLE = {'plot_id': 'pemfc_hydrogen_stoichiometry', 'format': 'svg', 'dpi': 150, 'figsize': None, 'rc': {}}
...
# ---------------------------------------------------------------------------
SERIES = [{'csv': 'pemfc_hydrogen_stoichiometry_Posterior_mean.csv', 'x': 'time', 'y': 'posterior_mean', 'err': '', 'x_err': 'time_error', 'label': 'Posterior mean'}]
...
def make_plot(title: str = 'Hydrogen stoichiometry') -> plt.Figure:
    ...
    ax.set_ylabel('Hydrogen stoichiometry [1]')
    ax.set_title(title)
    ax.legend()
