#!/usr/bin/env python3
"""Standalone plot — run with: python plot_pemfc_cathode_inlet_relative_humidity.py [-o output.svg]
...
# ---------------------------------------------------------------------------
VIPR_STYLE = {'plot_id': 'pemfc_cathode_inlet_relative_humidity', 'format': 'svg', 'dpi': 150, 'figsize': None, 'rc': {}}
...
# ---------------------------------------------------------------------------
SERIES = [{'csv': 'pemfc_cathode_inlet_relative_humidity_Posterior_mean.csv', 'x': 'time', 'y': 'posterior_mean', 'err': '', 'x_err': 'time_error', 'label': 'Posterior mean'}]
...
def make_plot(title: str = 'Cathode inlet relative humidity') -> plt.Figure:
    ...
    ax.set_ylabel('Cathode inlet relative humidity [1]')
    ax.set_title(title)
    ax.legend()
