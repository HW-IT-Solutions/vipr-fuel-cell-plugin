#!/usr/bin/env python3
"""Standalone plot — run with: python plot_pemfc_anode_inlet_relative_humidity.py [-o output.svg]
...
# ---------------------------------------------------------------------------
VIPR_STYLE = {'plot_id': 'pemfc_anode_inlet_relative_humidity', 'format': 'svg', 'dpi': 150, 'figsize': None, 'rc': {}}
...
# ---------------------------------------------------------------------------
SERIES = [{'csv': 'pemfc_anode_inlet_relative_humidity_Posterior_mean.csv', 'x': 'time', 'y': 'posterior_mean', 'err': '', 'x_err': 'time_error', 'label': 'Posterior mean'}]
...
def make_plot(title: str = 'Anode inlet relative humidity') -> plt.Figure:
    ...
    ax.set_ylabel('Anode inlet relative humidity [1]')
    ax.set_title(title)
    ax.legend()
