#!/usr/bin/env python3
"""Standalone plot — run with: python plot_pemfc_anode_inlet_temperature.py [-o output.svg]
...
# ---------------------------------------------------------------------------
VIPR_STYLE = {'plot_id': 'pemfc_anode_inlet_temperature', 'format': 'svg', 'dpi': 150, 'figsize': None, 'rc': {}}
...
# ---------------------------------------------------------------------------
SERIES = [{'csv': 'pemfc_anode_inlet_temperature_Posterior_mean.csv', 'x': 'time', 'y': 'posterior_mean', 'err': '', 'x_err': 'time_error', 'label': 'Posterior mean'}]
...
def make_plot(title: str = 'Anode inlet temperature') -> plt.Figure:
    ...
    ax.set_ylabel('Anode inlet temperature [K]')
    ax.set_title(title)
    ax.legend()
