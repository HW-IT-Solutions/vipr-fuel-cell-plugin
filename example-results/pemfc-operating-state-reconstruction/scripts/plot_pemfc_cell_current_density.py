#!/usr/bin/env python3
"""Standalone plot — run with: python plot_pemfc_cell_current_density.py [-o output.svg]
...
# ---------------------------------------------------------------------------
VIPR_STYLE = {'plot_id': 'pemfc_cell_current_density', 'format': 'svg', 'dpi': 150, 'figsize': None, 'rc': {}}
...
# ---------------------------------------------------------------------------
SERIES = [{'csv': 'pemfc_cell_current_density_Posterior_mean.csv', 'x': 'time', 'y': 'posterior_mean', 'err': '', 'x_err': 'time_error', 'label': 'Posterior mean'}]
...
def make_plot(title: str = 'Cell current density') -> plt.Figure:
    ...
    ax.set_ylabel('Cell current density [A/cm²]')
    ax.set_title(title)
    ax.legend()
